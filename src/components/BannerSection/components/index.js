...
const BannerSection = ({
    ...
    try {
      ...
      TRAVEL_PACKAGES_DATA.forEach((destination) => {
        if (
          ...
          const id =
            trip.tripId ?? trip.id ?? `${destination.destination_id}-${index}`;
          ...
          trips.push({
            ...
            href:
              destinationId && tripId
                ? `/trip?destinationId=${destinationId}&tripId=${tripId}`
                : undefined,
          });
          // });
        }
  ...
  const handlePrevCards = useCallback(() => {
    if (!featured || featured.length === 0) return;
    setCurrentCardIndex(
      (prevIndex) => (prevIndex - 1 + featured.length) % featured.length,
    );
  }, [featured]);
          ...
          <div className={styles.cardsRow}>
            {featured &&
              ...
              Array.from({ length: 4 }).map((_, i) => {
                const destination =
                  featured[(currentCardIndex + i) % featured.length];
