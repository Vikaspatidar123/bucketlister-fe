...
const PricingSummary = ({
  ...
  const fullPaymentDiscount =
    paymentType === "full" ? Math.min(totalPackageAmount * 0.1, 1500) : 0; // Up to 10% off, max ₹1500

  const discountedTotal = Math.max(
    totalPackageAmount - bookingDiscount - fullPaymentDiscount,
    0,
  ); // Ensure non-negative
  const bookingAmount = Math.max(
    paymentType === "full" ? discountedTotal : discountedTotal * 0.2,
    0,
  ); // Ensure non-negative
  ...
  return (
    <div
      className={`${styles.pricingSummaryContainer} ${compact ? styles.compact : ""}`}
    >
            ...
            <div className={styles.tripInfo}>
              ...
              {selectedBatchInfo && (
                <span className={styles.batchInfo}>
                  Batch: {selectedBatchInfo.dateRange}
                </span>
              )}
            </div>
          )}
      ...
      <div className={styles.breakdown}>
        ...
        {(bookingDiscount > 0 || fullPaymentDiscount > 0) && (
          <div className={styles.breakdownRow}>
            ...
            <div className={styles.value}>
              - ₹
              {(bookingDiscount + fullPaymentDiscount).toLocaleString("en-IN")}
            </div>
          </div>
        )}
